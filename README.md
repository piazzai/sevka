...
There is one problem with Iosevka's use on the web, though. Due to extensive Unicode coverage, language support, customization options, OpenType features, and ligature sets, its filesize is massive. Just the vanilla face (regular weight, normal width, upright slope, in TrueType format) is 9.9 MB. A minimal bundle that includes regular, italic, bold, and bold italic, necessary to render markdown documents like this, totals 40.4 MB. And if we simply use an off-the-shelf Iosevka package—say, [Iosevka Slab](https://github.com/be5invis/Iosevka/releases/download/v33.3.3/PkgTTF-IosevkaSlab-33.3.3.zip)—with all bells and whistles, we are looking at a whopping 546.1 MB. This is not acceptable for a website, especially if it doesn't even need full Unicode encoding and only needs to render ASCII or Latin characters.
